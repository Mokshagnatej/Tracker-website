...
export default function TasksPage({ tasks, onToggle, onAdd, onDelete, showToast }: Props) {
  const [filter, setFilter] = useState<Filter>("all");
  const [newName, setNewName] = useState("");
  const today = todayStr();

  const doneToday = tasks.filter((h) => h.history[today]).length;
  const total = tasks.length;
  const bestStreak = Math.max(0, ...tasks.map((h) => h.streak));
  const rate = total > 0 ? Math.round((doneToday / total) * 100) : 0;

  const filtered = useMemo(() => {
    if (filter === "done")    return tasks.filter((h) =>  h.history[today]);
    if (filter === "pending") return tasks.filter((h) => !h.history[today]);
    return tasks;
  }, [tasks, filter, today]);
  ...
  const weeklyData = useMemo(() =>
    Array.from({ length: 7 }, (_, i) => {
      const d = dateStr(6 - i);
      return { day: dayLabel(6 - i), done: tasks.filter((h) => h.history[d]).length };
    }),
    [tasks]
  );
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1.25rem" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "2rem", flexWrap: "wrap" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem", flex: 1, minWidth: 160 }}>
            {[
              ...
              { l: "Tasks Total", v: s(total),         em: "◎",  c: "var(--gold)" },
      ...
      <div className="card" style={{ padding: "1.25rem 1.5rem" }}>
        <div className="label" style={{ marginBottom: "0.85rem" }}>New Task</div>
      ...
      <div className="card" style={{ overflow: "hidden" }}>
        ...
        {filtered.length === 0 ? (
          <div style={{ padding: "3rem 1.5rem", textAlign: "center", color: "var(--text-4)", fontStyle: "italic", fontSize: "0.85rem" }}>
            {filter === "done" ? "Nothing completed yet." : filter === "pending" ? "All done — excellent work." : "Add your first task."}
          </div>
        ) : (
